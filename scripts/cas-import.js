...
class foo {
  ...
  async run() {
    let src_creds, src_ctx, dst_ctx;
    let dst_container, src_containers;

    let dst_list = await Storage.getFileList(dst_ctx, dst_container);
    dst_list = dst_list.map(entry => path.basename(entry.name));

    let src_list = [];
    for(let src_container of src_containers) {
      let tmp = await Storage.getFileList(src_ctx, src_container);
      tmp.forEach(entry => entry.container = src_container);
      src_list.push(...tmp);
    ...
    let push_file = async (entry) => {
      ...
      try {
        let res = await Storage.put(dst_ctx, dst_container, dst_name, {headers})
      } catch(err) {
        ...
        do {
          ...
          try {
            let tmp = await Storage.head(dst_ctx, dst_container, dst_name);
            break;
          } catch(err) {
